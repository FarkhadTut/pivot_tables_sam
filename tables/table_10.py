...
def concat():
    ...
    df_total = add_sum_formula(df_total, "ЖАМИ", col_total_idx=2, sum_start_col_idx=3)
...
def crop_data(file, sheet_name):
    ...
    row_start, col_start = getIndexes(df, 'Аҳоли сони')[0]
    ...
    data_cols = columns[2:]
    mask_non_empty = ~df_out[data_cols].isna().all(axis=1)
...
def add_headers(df_total, file_startswith, sheet_name=0):
    ...
    row, col = getIndexes(df_template, 'Аҳоли сони')[0]
...
def add_total_row(df_out):
    ...
    if df_out.at[0, columns[2]] == 'Ҳудуд бўйича - жами':
        df_out = df_out.tail(-1)

    df_out = pd.concat([df_out.head(1), df_out], axis=0)
    df_out.reset_index(inplace=True, drop=True)
    df_out.iloc[0] = pd.NA
    df_out.at[0, columns[2]] = "Жами"
    df_out.at[0, columns[1]] = df_out.at[1, columns[1]]
    return df_out