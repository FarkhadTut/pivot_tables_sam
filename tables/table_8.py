...
def crop_data(file, sheet_name):
    ...
    row_start, col_start = getIndexes(df, 'Асбоб-ускуна тури')[0]
    ...
    cols_data = columns[2:]
    mask_non_empty = ~df_out[cols_data].isna().all(axis=1)
...
def add_headers(df_total, file_startswith, sheet_name=0):
    ...
    row, col = getIndexes(df_template, 'Асбоб-ускуна тури')[0]
